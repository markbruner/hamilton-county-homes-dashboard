...
def scrape_table_by_xpath(wait, xpath):
    ...
    """    
    if not xpath:
        logging.error("XPath is empty. Cannot locate element.")
        return pd.DataFrame()
    try:
        ...
        logging.error(f"Table at {xpath} not found: {e}")
        return pd.DataFrame()
    except ValueError as e:
        logging.error(f"Failed to parse table at {xpath}: {e}")
        return pd.DataFrame()
...
def scroll_and_click(driver, wait, element):
    ...
    try:
        ...
        logging.warning(f"Element not interactable. Using JS click: {e}")
        driver.execute_script("arguments[0].click();", element)

    except Exception as e:
        logging.error(f"Failed to interact with element: {e}")
...
def transform_table(table):
    if table.empty:
        logging.warning("Received an empty table for transformation.")
